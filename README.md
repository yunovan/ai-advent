...
3. **Длинный диалог**  
   Задайте 5–10 длинных вопросов в одном диалоге → промпт и полоса использования лимита растут. Откройте **«Рост токенов/цены»** — таблица, где с каждым ходом `промпт, tok`, `сумма tok` и `сумма, $` монотонно растут: история пересылается целиком на каждом ходу, поэтому диалог дорожает.
