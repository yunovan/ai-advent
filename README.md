...
Агент считает токены и показывает, как они влияют на поведение на протяжении длинного диалога:

1. **токены текущего запроса** (`requestTokens`);
2. **токены всей истории диалога** (`historyTokens`) — сколько уже «накопилось» до этого сообщения;
3. **токены ответа модели** (`responseTokens`).
...
- **стоимость** — цена хода и накопленная цена всего диалога, считается из токенов и цен `day8.input-price` / `day8.output-price` (USD за 1 млн токенов).

Порядок работы: загружаем сессию → считаем токены истории → добавляем сообщение → считаем «промпт» (история + запрос) → **если он больше контекстного окна `day8.context-limit`, LLM не вызывается** — вместо этого возвращается ответ с флагом `exceeded: true` и объяснением. Так же поступила бы и сама модель (ошибка `context_length_exceeded`), но агент ловит переполнение заранее, не тратя запрос.

### Что показать на видео

1. **Задача дня**  
   «Агент считает токены запроса, истории и ответа. Смотрим, как растут токены и стоимость по мере диалога — и что ломается при переполнении.»

2. **Короткий диалог**  
   http://localhost:8080/day8.html → «Привет! Меня зовут Ася.» → видно `запрос ~N · история ~0 · промпт ~M · ответ ~K`. Индикатор заполнения лимита почти пустой.

3. **Длинный диалог**  
   Задайте 5–10 длинных вопросов в одной сессии → промпт и полоса использования лимита растут. Откройте **«Статистика сессии»** — таблица, где с каждым ходом `промпт, tok`, `сумма tok` и `сумма, $` монотонно растут: цена диалога растёт, потому что история пересылается целиком на каждом ходу.
...
./gradlew bootRun --args="--day=8 --prompt="Привет, меня зовут Ася" --cli"

# таблица роста токенов/цены по сессии
./gradlew bootRun --args="--day=8 --table --cli"

# показать переполнение лимита (маленький лимит)
./gradlew bootRun --args="--day=8 --prompt="Длинное сообщение..." --limit=60 --cli"

# сбросить сессию
./gradlew bootRun --args="--day=8 --reset --cli"
```
...
curl -s http://localhost:8080/api/day8/chat \
  -H 'Content-Type: application/json' \
  -d '{"sessionId":"default","request":"Привет, меня зовут Ася","contextLimit":128000}'

# таблица роста токенов и стоимости по всей сессии
curl -s "http://localhost:8080/api/day8/metrics?sessionId=default"

# сброс истории
curl -s http://localhost:8080/api/day8/reset \
  -H 'Content-Type: application/json' \
  -d '{"sessionId":"default"}'
```
...
| `day08/Day08TokenService.java` | сервис: чат + метрики токенов/цены + защита от превышения лимита, таблица роста `metrics()` |
| `day08/Day8Properties.java` | `day8.context-limit`, `day8.input-price`, `day8.output-price` |
| `day08/Day08TokenController.java` | `GET/POST /api/day8/chat`, `GET /api/day8/metrics`, `/api/day8/reset` |
| `day08/Day08CliRunner.java` | CLI: `--day=8 --prompt/--session/--limit/--table/--reset` |
| `static/day8.html` | чат с метриками, полосой лимита и таблицей роста сессии |
